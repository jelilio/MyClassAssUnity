...
public class NinjaMovementScript : MonoBehaviour
{
    ...
    void Move()
    {
        ...
        if (Mathf.Abs(Input.GetAxis("Horizontal")) > 0.1f &&
            ...
            maxSpeed) // if the absolute value of the input is greater than 0.1, and player is not moving faster than max Speed
        {
            myRb.AddForce(new Vector2(Input.GetAxis("Horizontal") * acceleration, 0),
                ForceMode2D.Force); //gets Input value and multiplies it by acceleration in the x direction.
        }
    }
    ...
    void Jump()
    {
        ...
        {
            myRb.AddForce(new Vector2(0, secondaryJumpForce),
                ForceMode2D.Force); //while the jump button is held, add a force in the y direction
